...
const Header = ({ showHamburger }: { showHamburger: boolean }) => {
  ...
  //     try {
  //       const user: CognitoTokenPayload = await getCurrentUser();
  //       setUserData({
  ...
  return (
    <>
            ...
            {login ? (
              <div className="ml-6 flex justify-between items-center gap-5">
                ...
                <Link
                  ...
                  className="border-2 w-full border-[#E5E5E5] rounded-[60px]  flex items-center"
                >
                  <p
      ...
      <MobileHeader isLogin={login} showHamburger={showHamburger} />
    </>
  );
};
