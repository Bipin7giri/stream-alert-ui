...
import { usePathname } from "next/navigation";
import { RoutesIF, StreamerDashboardRoutes } from "../../../routes/routes";
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  const MenuItem = ({
    ...
    console.log(routeSplit + "route split: ");
    return isClickAble === false ? (
      <Link
        href={route}
        className={`${
          pathSplit[pathSplit.length - 1] === routeSplit[routeSplit.length - 1]
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <Link href="/" className="p-10 flex items-center gap-[10px]">
      ...
      <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        ...
        <nav className=" px-4  lg:px-6">
          ...
          <div className="flex flex-col w-full">
            {StreamerDashboardRoutes.map((item: RoutesIF, id: number) => {
              return (
                <MenuItem
                  key={id}
                  name={item.name}
